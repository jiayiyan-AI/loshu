...
一条关联只在持有外键的那一侧声明，目标表上不为它另立一个反向列。**逆向方向不是一个需要声明的对象** —— 它是外键白拿的另一半（见 5.3），由查询层按需走出来：视图汇总子行时，`via` 引用的就是这条正向 relation 的 id（见 8.2），方向由「站在目标表一侧回看」自然确定。

给反向另立一条声明换不到任何东西，却要付代价：「建一个双向关联」就得拆成两侧各一条，两次提交之间会出现「反向端指向一个不存在的列」这种不合法的中间状态。**关系只存在持有外键的那一侧，只声明一次。**
